<script lang="ts">
    ...
    let cipherId = 142;
    let cipher = 'qefp lkb fp xk bxpv lkb qebv tfii pqxoq ql dbq exoabo';
    let solvers = 8732;
    let avgTime = '14:08';
...
<div class="relative bg-surface rounded-xs pt-6.5 px-7.5 pb-5.5 mt-10">
    ...
    <div class="flex justify-between items-center pt-3 font-mono text-[10.5px] text-muted tracking-[0.06em]">
        {#key key}
            <span>{moment().format('Do MMM YYYY').toLowerCase()} · 00:00 utc</span>
            <span>resets in {moment().endOf('day').diff(moment(), 'hours')}h {moment().endOf('day').diff(moment(), 'minutes') % 60}m {moment().endOf('day').diff(moment(), 'seconds') % 60}s</span>
        {/key}
    </div>
